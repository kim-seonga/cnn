...
def imageTest(filename):
    myImage = Image.open(filename)
    npMyImage = np.zeros([1,784])
    npMyImage1 = (np.array(myImage))
    cnt = 0

    for i in range(28):
        for j in range(28):
            if npMyImage1[i,j] == True:
                npMyImage[0,cnt] = 0.0
                cnt=cnt+1
            else:
                npMyImage[0,cnt] = 1.0
    ...
    m = sess.run(y_conv,feed_dict = {x:npMyImage, keep : 1.0})
    print(m)
    result = sess.run(tf.argmax(m,1))
    print(result)
